...
public class CustomerDao extends Dao {
	...
	public void load() throws ApplicationException {
		...
		try {
			if (!Database.tableExists(CustomerDao.CUSTOMER_TABLE_NAME) ) {
	...
	public void update(Customer customer) throws SQLException {
		...
		boolean result = execute(sqlString, customer.getFirstName(), customer.getLastName(), customer.getStreet(),
				customer.getCity(), customer.getPostalCode(), customer.getPhone(), customer.getEmailAddress(), toTimestamp(customer.getJoinedDate()), customer.getId());
	...
	public int countAllCustomers() throws Exception {
		...
		try {
			...
			statement = connection.createStatement();
			// Execute a statement
			String sqlString = String.format("SELECT COUNT(*) AS total FROM %s", CUSTOMER_TABLE_NAME);
	...
	public static Map<Long, Customer> getAllCustomers() throws Exception {
		Statement statement = null;
		...
		try {
			...
			statement = connection.createStatement();
			// Execute a statement
			String sqlString = String.format("SELECT * FROM %s", CUSTOMER_TABLE_NAME);
			ResultSet resultSet = statement.executeQuery(sqlString);
			while (resultSet.next()) {
				Timestamp timestamp = resultSet.getTimestamp(Column.JOINED_DATE.name);
				LocalDate date = timestamp.toLocalDateTime().toLocalDate();
